...
fn parse_games(input: &str) -> Vec<Game> {
    ...
    for line in input.lines() {
        ...
        let id = tokens[0].split([' ']).collect::<Vec<&str>>()[1]
            .parse::<u32>()
            .unwrap();
        let roll_strs = tokens[1].split(';').collect::<Vec<&str>>();
        let mut rolls = Vec::<Roll>::new();
        for roll_str in roll_strs {
            let mut red = 0;
            let mut green = 0;
            let mut blue = 0;
            for color_str in roll_str.split(',').collect::<Vec<&str>>() {
                let color_str = &color_str[1..];
                let components = color_str.split(' ').collect::<Vec<&str>>();
                assert_eq!(components.len(), 2);
                let value: u32 = components[0].parse().unwrap();
                match components[1] {
                    "red" => red = value,
                    "green" => green = value,
                    "blue" => blue = value,
            ...
            rolls.push(Roll { red, green, blue })
        }
        games.push(Game { id, rolls })
    }
    games
}

pub fn day_2_pt_1() -> String {
    ...
    for game in games {
        ...
        for roll in game.rolls {
            if roll.red > red_cubes || roll.blue > blue_cubes || roll.green > green_cubes {
                valid = false;
            }
        }
        if valid {
...
pub fn day_2_pt_2() -> String {
    ...
    for game in games {
        ...
        let mut max_blue = 0;
        for roll in game.rolls {
            max_red = cmp::max(max_red, roll.red);
            max_green = cmp::max(max_green, roll.green);
            max_blue = cmp::max(max_blue, roll.blue);
        }
...
static _SAMPLE_INPUT: &str = "Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";

static INPUT: &str = "Game 1: 4 red, 5 blue, 9 green; 7 green, 7 blue, 3 red; 16 red, 7 blue, 3 green; 11 green, 11 blue, 6 red; 12 red, 14 blue
