    ...
    for line in INPUT.lines() {
        ...
        let mut digits: Vec<(usize, u32)> = line
            ...
            .filter_map(|digit| {
                index += 1;
                if digit.is_ascii_digit() {
                    Some((index, digit.to_digit(10).unwrap()))
                } else {
                    None
                }
            })
        ...
        for number in numbers {
            let mut offset = 0;
            while let Some(start_index) = slice[offset..].find(number.1) {
                digits.push((start_index + offset, number.0));
...
static INPUT: &str = "gsjgklneight6zqfz
